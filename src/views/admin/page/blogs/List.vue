<template>
    <Modal v-model:open="modalOpen" :modalStatus="modalStatus" :modalTitle="modalTitle" :width="modalWidth"
        :height="modalHeight" :maskClosable="false" :handleOk="handleOk" :handleCancel="handleCancel">
        <template v-slot:form>
            <a-form :model="formState" name="basic" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"
                ...
                <a-form-item label="分类：" name="classId" :rules="[{ required: false, message: '未选择任何选项' }]">
                    <Select :options="formClassList" style="width:300px" v-model:value="formState.classId"></Select>
                </a-form-item>
